...
namespace ChapinCine.Vista
{
    ...
    public class Menu
    {
        ...
        /// <summary>Construye el código DOT de los reportes gráficos.</summary>
        private GeneradorDot generador;

        /// <summary>Escribe el .dot en disco e invoca a Graphviz.</summary>
        private EjecutorGraphviz graphviz;
                    ...
                    PilaReservaciones pilaReservaciones)
        {
            ...
            generador = new GeneradorDot();
            graphviz = new EjecutorGraphviz();
        }
        ...
        public void Iniciar()
        {
            ...
            while (!salir)
            {
                ...
                switch (opcion.Trim())
                {
                    ...
                    case "10":
                        OpcionReporteReservacion();
                        break;

                    case "11":
                        OpcionReporteEstructura();
                        break;
        ...
        private void MostrarOpciones()
        {
            ...
            Console.WriteLine("  9. Reporte grafico de una sala        (Graphviz)");
            Console.WriteLine(" 10. Reporte grafico de una reservacion (Graphviz)");
            Console.WriteLine(" 11. Reporte de la estructura interna   (Graphviz)");
            Console.WriteLine(" 12. Eliminar una sala");
            Console.WriteLine(" 13. Ver historial de operaciones");
            Console.WriteLine(" 14. Simular fila de taquilla");
        ...
        /// <summary>
        /// Muestra el resultado de una generación de reporte y ofrece abrir
        /// la imagen. Concentra el manejo del resultado para que las tres
        /// opciones de reporte no repitan la misma lógica.
        /// </summary>
        private void MostrarResultadoReporte(ResultadoReporte resultado)
        {
            Console.WriteLine();

            if (!resultado.DotGenerado)
            {
                Console.WriteLine(">> ERROR: " + resultado.MensajeError);
                Pausa();
                return;
            ...
            Console.WriteLine("   " + resultado.RutaDot);

            if (!resultado.ImagenGenerada)
            {
                Console.WriteLine();
                Console.WriteLine(">> No se generó la imagen.");
                Console.WriteLine("   " + resultado.MensajeError);
                Console.WriteLine();
                Console.WriteLine("   Puede generarla manualmente con:");
                Console.WriteLine("   dot -Tpng \"" + resultado.RutaDot + "\" -o salida.png");
                Pausa();
                return;
            }

            Console.WriteLine(">> Imagen generada:");
            Console.WriteLine("   " + resultado.RutaImagen);
            Console.WriteLine();
            Console.Write("¿Desea abrir la imagen? (s/n): ");

            string respuesta = Console.ReadLine();

            if (respuesta != null && respuesta.Trim().ToLower() == "s")
            {
                if (!graphviz.AbrirImagen(resultado.RutaImagen))
                {
        ...
        /// <summary>
        /// Genera el reporte gráfico de una sala completa.
        ///
        /// El generador recorre la malla con los mismos métodos que utiliza
        /// el dibujo en consola; lo único que cambia es el formato de
        /// salida. Ni los TDA ni el modelo se modificaron para producir
        /// este reporte.
        /// </summary>
        private void OpcionReporteSala()
        {
            Console.WriteLine();
            Console.WriteLine("--- REPORTE GRAFICO DE UNA SALA ---");
            ...
            string contenido = generador.GenerarSala(sala);
            string nombreArchivo = "sala_" + LimpiarNombre(sala.Nombre);

            ResultadoReporte resultado = graphviz.Generar(contenido, nombreArchivo, "dot");

            historial.Apilar("Reporte gráfico de la sala: " + sala.Nombre);
            MostrarResultadoReporte(resultado);
        }

        /// <summary>
        /// Genera el reporte gráfico de una reservación, con sus asientos
        /// resaltados y los datos de la operación al pie.
        /// </summary>
        private void OpcionReporteReservacion()
        {
            Console.WriteLine();
            Console.WriteLine("--- REPORTE GRAFICO DE UNA RESERVACION ---");

            if (reservaciones.EstaVacia())
            {
                Console.WriteLine("No hay reservaciones registradas. Use la opción 6 primero.");
                Pausa();
                return;
            }

            // Se listan las reservaciones recorriendo la pila sin extraerlas.
            NodoReservacion actual = reservaciones.VerCima();
            int numero = 1;
            ...
            Console.WriteLine();
            Console.Write("Seleccione el número de la reservación: ");

            string entrada = Console.ReadLine();
            int seleccion;

            if (!int.TryParse(entrada, out seleccion)
                || seleccion < 1 || seleccion > reservaciones.Cantidad)
            {
                Console.WriteLine(">> Selección no válida.");
                Pausa();
                return;
            ...
            Reservacion reservacion = actual.Dato;

            string contenido = generador.GenerarReservacion(reservacion);
            string nombreArchivo = "reservacion_" + LimpiarNombre(reservacion.Cliente.Nombre)
                                   + "_" + reservacion.Fila + "_" + reservacion.ColumnaInicial;

            ResultadoReporte resultado = graphviz.Generar(contenido, nombreArchivo, "dot");

            historial.Apilar("Reporte gráfico de reservación: " + reservacion.ObtenerDescripcion());
            MostrarResultadoReporte(resultado);
        }

        /// <summary>
        /// Genera el reporte de la estructura interna de la matriz
        /// ortogonal sobre un fragmento de la malla, dibujando un nodo por
        /// celda y una arista por cada enlace entre vecinos.
        ///
        /// Se limita a un fragmento porque una malla completa produciría
        /// una imagen demasiado densa para leerse.
        /// </summary>
        private void OpcionReporteEstructura()
        {
            Console.WriteLine();
            Console.WriteLine("--- REPORTE DE LA ESTRUCTURA INTERNA ---");
            ...
            Console.WriteLine();
            Console.WriteLine("Indique el fragmento a dibujar. Se recomienda un área");
            Console.WriteLine("de pocas filas y columnas para que la imagen sea legible.");
            Console.WriteLine();

            int filaIni = LeerEntero("Fila inicial", 1, sala.Asientos.TotalFilas);
            int filaFin = LeerEntero("Fila final", 1, sala.Asientos.TotalFilas);
            int colIni = LeerEntero("Columna inicial", 1, sala.Asientos.TotalColumnas);
            int colFin = LeerEntero("Columna final", 1, sala.Asientos.TotalColumnas);

            if (filaIni < 0 || filaFin < 0 || colIni < 0 || colFin < 0)
            {
            ...
            string contenido = generador.GenerarEstructura(sala, filaIni, colIni, filaFin, colFin);

            if (contenido == null)
            {
                Console.WriteLine(">> El rango indicado no es válido.");
                Pausa();
                return;
            }

            string nombreArchivo = "estructura_" + LimpiarNombre(sala.Nombre);

            // Este reporte define posiciones fijas, por lo que se procesa
            // con neato en lugar de dot.
            ResultadoReporte resultado = graphviz.Generar(contenido, nombreArchivo, "neato");

            historial.Apilar("Reporte de estructura de: " + sala.Nombre);
            MostrarResultadoReporte(resultado);
        }

        /// <summary>
        /// Solicita un número entero dentro de un rango. Devuelve -1 si el
        /// valor ingresado no es válido.
        /// </summary>
        private int LeerEntero(string etiqueta, int minimo, int maximo)
        {
            Console.Write(etiqueta + " (" + minimo + " a " + maximo + "): ");
        ...
        /// <summary>
        /// Sustituye por guion bajo los caracteres que no conviene usar en
